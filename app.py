...
# --- هذا هو الجزء الذي تم إصلاحه وإضافته ---
...
def analyze_report():
    try:
        ...
        1.  `interpretation`: (String) شرح احترافي ومبسط في نفس الوقت لما يظهر في التقرير. ركز على المؤشرات الرئيسية غير الطبيعية إن وجدت (مثال: "يُظهر التقرير ارتفاعاً في إنزيمات الكبد (ALT)، مما قد يشير إلى وجود إجهاد على الكبد. كما أن مستوى السكر التراكمي (HbA1c) ضمن المعدل الطبيعي."). **لا تقدم تشخيصاً نهائياً أبداً وقل دائماً أن هذه ملاحظات أولية.**
        2.  `temporary_advice`: (Array of strings) قائمة نصائح عامة ومؤقتة يمكن للمريض اتباعها حتى زيارة الطبيب. يجب أن تكون نصائح آمنة ومبنية على التحليل (مثال: "يُنصح بالراحة وتجنب الأطعمة الدهنية مؤقتاً.", "الحرص على شرب كميات كافية من الماء.", "مراقبة أي أعراض جديدة قد تظهر.").
        3.  `recommendations`: (Array of objects) قائمة تحتوي على **عيادة واحدة فقط** هي الأنسب للحالة. يجب أن يحتوي كل عنصر على:
            - `id`: معرف (ID) العيادة من القائمة المتاحة (مثال: "جهاز-هضمي-ومناظير").
            - `reason`: (String) شرح بسيط ومباشر لسبب اختيار هذه العيادة (مثال: "بناءً على ارتفاع إنزيمات الكبد، نوصي بالتوجه لعيادة الجهاز الهضمي لتقييم الحالة بشكل دقيق.").

        **مهم جداً:** إذا كانت الملفات غير واضحة أو لا يمكن قراءتها، أعد رداً مناسباً في حقل `interpretation` مثل "الملفات المرفقة غير واضحة أو لا تحتوي على معلومات طبية يمكن تحليلها." واترك الحقول الأخرى فارغة.
        """
